...
String serverName = "http://172.20.10.6:3001/api/history-data/add";
// String serverName = "http://192.168.1.2:1880/test";

unsigned long lastTimeCallApi = 0;
unsigned long timerDelayCallApi = 10000;
...
const char* ssid = "Binhb";              // Tên Wifi
...
String createMessage() {
  ...
  myObject["light_setting_value"] = light_setting_value;
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if (strstr(topic, topic_master_sub.c_str()) != null) {
    ...
    if (myObject.hasOwnProperty("light_setting_value")) {
      light_setting_value = (double)myObject["light_setting_value"];
    }
...
void autoMode() {
  if (light_sensor < light_setting_value) {
    relay_2 = 1;
  }
  else {
    relay_2 = 0;
